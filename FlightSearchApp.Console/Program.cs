...
namespace FlightSearchApp.ConsoleApp
{
    class Program
    {
        static async Task Main(string[] args)
        {
            ...
            // Add other services
            services.AddTransient<IFlight_Service, Flight_Service>();
            services.AddTransient<IHotel_Service, Hotel_Service>();
            ...
            services.AddTransient<IValidator<FlightSearchRequest>, FlightSearch>();
            services.AddTransient<IValidator<HotelSearchRequest>, HotelSearch>();
            ...
            try
            {
                ...
                hotelRequest.City = Console.ReadLine();
                ...
                if (!string.IsNullOrEmpty(hotelRequest.City))
                {
                    var hotels = await serviceProvider.GetRequiredService<IHotel_Service>()
                    .SearchHotelsAsync(hotelRequest.City);

                    Console.WriteLine("Hotels:");
                    foreach (var hotel in hotels)
                    {
                        Console.WriteLine($"{hotel.Name} in {hotel.City}, Price per night: {hotel.PricePerNight}, Rating: {hotel.Rating}");
                    }
                }
            }
            catch (Exception ex)
        ...
        private static async Task SearchOneWayFlights(IServiceProvider serviceProvider, IValidator<FlightSearchRequest> validator)
        {
            ...
            {
                flightRequest.Departure_Date = departureDate;
            }
            ...
            {
                var flights = await serviceProvider.GetRequiredService<IFlight_Service>()
                .SearchFlightsAsync(flightRequest.Origin, flightRequest.Destination, flightRequest.Departure_Date);
        ...
        private static async Task SearchRoundTripFlights(IServiceProvider serviceProvider, IValidator<FlightSearchRequest> validator)
        {
            ...
            {
                var flights = await serviceProvider.GetRequiredService<IFlight_Service>()
                .SearchFlightsAsync(flightRequest.Origin, flightRequest.Destination, flightRequest.Departure_Date, returnDate, true);
